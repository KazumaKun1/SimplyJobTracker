...
This is an Xcode project (no SPM/CocoaPods dependencies, no SwiftLint/SwiftFormat config). Use `xcodebuild` from the repo root, or open `SimplyJobTracker.xcodeproj` in Xcode.
...
- `TabCoordinator` (`Common/Coordinators`) is the root object, created once in `SimplyJobTrackerApp` with the shared `ModelContainer`. It owns one coordinator per tab (`HomeCoordinator`, `SettingsCoordinator`).
- `AppCoordinatorView` renders the `TabView` and, for each tab, wraps that tab's coordinator in a `NavigationStack(path:)` + `.navigationDestination(for:)`, and a `.sheet(item:)`.
- Two small generic protocols in `Common/Coordinators/Common` drive this, and every feature coordinator conforms to them:
  ...
  - `AlertCoordinator`: `presentAlert: AlertConfig?` plus an `alertQueue`, so alerts triggered while one is already showing are queued and presented in order (see `HomeCoordinator`).
- Feature coordinators also define a sheet route enum (e.g. `HomeSheetRoute`) and a `build(sheet:)` builder, following the same pattern as routes.
- View models (`@Observable`, e.g. `HomeViewModel`) hold a `weak` reference back to their coordinator to trigger navigation/alerts, and hold the feature's service for data operations. They never touch SwiftData or navigation state directly.

**Data layer**: SwiftData `@Model` classes live under each feature's `Model/` folder (`JobApplication`, `Interview`, both under `Features/Home/Main/Model`). Mutations go through an actor-isolated service protocol, not directly through a `ModelContext` from the view layer:
- `JobApplicationService` is a `protocol: Actor`; `JobApplicationServiceImpl` is a `@ModelActor actor` conforming to it, constructed per-coordinator from the shared `ModelContainer`. This keeps SwiftData writes off the main actor while view models stay `@Observable`/MainActor.
- Views read data directly via `@Query` (see `HomeView`), and go through the service only for writes (create/delete).

**View file convention**: each screen is a `FeatureNameView.swift` containing only the top-level `body`, with all of its subviews defined as nested types in a matching `FeatureNameView+Extensions.swift` (e.g. `HomeView+Extensions.swift`, `EditJobApplicationView+Extensions.swift`). When adding subviews to an existing screen, put them in the `+Extensions.swift` file as `extension FeatureNameView { struct SomeSubview: View { ... } }`, grouped under `// MARK:` sections — don't create new standalone view files for screen-local subviews.
...
**Feature module layout** (see `Features/Home`): each feature is split into `Coordinators/`, `Model/`, `View/` (with a `View/Extensions/` subfolder for the `+Extensions.swift` files), and `ViewModel/`. New features should follow this same folder shape.
