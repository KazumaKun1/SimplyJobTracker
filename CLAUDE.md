...
- **Data sharing, not duplication**: the widget does not have its own copy of the model layer. `Common/Enums/JobApplicationStatus.swift`, `Common/Resources/SharedModelContainer.swift`, and `Features/Home/Model/JobApplication.swift` are added to the widget target as well as the main app target (Xcode file-system-synchronized group membership exceptions in `project.pbxproj`) — if you rename or move any of these three files, re-check that target membership in Xcode rather than assuming the synchronized group carries it over automatically.
